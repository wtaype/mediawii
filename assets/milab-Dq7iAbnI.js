import{j as t}from"./vendor-gzd0YkcT.js";const g=(a,c,d={})=>{const{stagger:e=0,anim:_="",threshold:f=.1,once:p=!0}=d,r=[...document.querySelectorAll(a)];if(!r.length)return;_&&r.forEach(l=>l.classList.add(_));const b=new IntersectionObserver(l=>l.filter(s=>s.isIntersecting).forEach(s=>{const i=r.indexOf(s.target);setTimeout(()=>{_&&s.target.classList.add("wi_visible"),c?.(s.target,i)},e*i),p&&b.unobserve(s.target)}),{rootMargin:"20px",threshold:f});r.forEach(l=>b.observe(l))};g.v="10.1";const m=(a,c,{stagger:d=0,anim:e="",threshold:_=.1,once:f=!0,root:p=null,onExit:r=null,delay:b=0}={})=>{const l=[...document.querySelectorAll(a)];if(!l.length)return null;const s=new IntersectionObserver(i=>i.forEach(n=>{const v=l.indexOf(n.target);n.isIntersecting?(setTimeout(()=>{e&&n.target.classList.add("wi_visible"),c?.(n.target,v)},b+d*v),f&&s.unobserve(n.target)):r?.(n.target,v)}),{rootMargin:"20px",threshold:_,root:p});return l.forEach(i=>{e&&i.classList.add(e),s.observe(i)}),s};m.v="11.0";const k=()=>`
<div class="mlab_wrap">
...
</div>`;let o=null,u=0;const w=()=>{g("#d10_stagger .mlab_card",null,{anim:"wi_fadeUp",stagger:120}),g("#d10_cnt",()=>{const a=t("#cnt10"),c=+a.data("target");let d=0;const e=setInterval(()=>{d+=c/50,d>=c?(a.text(c),clearInterval(e)):a.text(Math.floor(d))},30)}),m("#d11_stagger .mlab_card",null,{anim:"wi_fadeUp",stagger:120}),m("#d11_exit .mlab_card_exit",a=>{u++,t("#entry_cnt").text("×"+u)},{anim:"wi_fadeUp",once:!1,onExit:a=>a.classList.remove("wi_visible")}),o=m("#d11_obs .mlab_card",()=>{t("#obs_status").removeClass("mlab_obs_off").html('<i class="fas fa-circle"></i> activo')},{anim:"wi_fadeUp",once:!1,onExit:a=>{a.classList.remove("wi_visible"),o&&t("#obs_status").html('<i class="fas fa-circle"></i> activo · espera')}}),m("#d11_delay .mlab_card",null,{anim:"wi_fadeUp",stagger:150,delay:600}),t(document).on("click.mlab","#btn_disc",function(){o&&(o.disconnect(),o=null,t("#obs_status").addClass("mlab_obs_off").html('<i class="fas fa-circle"></i> desconectado'),t(this).html('<i class="fas fa-check"></i> Observer desconectado — ya no responde').prop("disabled",!0).addClass("mlab_btn_done"))})},x=()=>{o?.disconnect(),o=null,u=0,t(document).off(".mlab")};export{x as cleanup,w as init,k as render};
